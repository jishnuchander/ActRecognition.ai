...
activity = cell(no_of_frames,1);                            % cell is an array which can hold diff data types

if fid == -1
    disp(msg);
else
    disp(['Reading ' filename ' ...']);
    example = struct;                                       % declaring example to be of struct data type 
    example.type = sscanf(filename, '%[a-z A-Z]');          % adding field "type" to struct
